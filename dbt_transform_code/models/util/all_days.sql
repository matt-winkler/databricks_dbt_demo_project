with rawdata as (
    ...
    unioned as (
        select p.generated_number * pow(2, 0)
        +
    ...
    select (
        to_timestamp(
            to_unix_timestamp(
                date_add(
                    coalesce(
                        date(to_date('01/01/1992', 'mm/dd/yyyy')),
                        nvl2(
                            date(to_date('01/01/1992', 'mm/dd/yyyy')),
                            assert_true(date(to_date('01/01/1992', 'mm/dd/yyyy')) is not null),
                            null
                        )
                    ),
            ...
            + to_unix_timestamp(
                coalesce(
                    to_timestamp(to_date('01/01/1992', 'mm/dd/yyyy')),
                    nvl2(
                        to_timestamp(to_date('01/01/1992', 'mm/dd/yyyy')),
                        assert_true(to_timestamp(to_date('01/01/1992', 'mm/dd/yyyy')) is not null),
                        null
                    )
                )
            )
