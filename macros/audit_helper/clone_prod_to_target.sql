{% macro 
   ...
   clone_prod_to_target(
    ...
    prod_database='matt_winkler_prod', 
...
%}
  ...
  {% set table_sql %}
    show tables in {{prod_database}}.{{prod_schema}};
  {% endset %}

  {% set view_sql %}
    show views in {{prod_database}}.{{prod_schema}};
  {% endset %}

  {% set view_use_sql %}
     use catalog {{prod_database}};
   ...
   {% if table_name not in view_results %}
     {% set sql %}
       ...
       create or replace table {{target_database}}.{{target_schema}}.{{table_name}}
        deep clone {{prod_database}}.{{prod_schema}}.{{table_name}};
